...
`sudo apt-get install autoconf automake libtool curl make g++ unzip valgrind cmake`
...
Alternatively, you may download and unzip from source: `get https://github.com/google/googletest/archive/release-1.10.0.zip`
...
cd protobuf
git checkout v3.5.1
Alternatively, you may download and unzip from source: `wget https://github.com/protocolbuffers/protobuf/releases/download/v3.5.1/protobuf-all-3.5.1.zip`
`unzip protobuf-all-3.5.1.zip`
Next, build protobuf:
`./autogen.sh`
`./configure`
`sudo make -j #cores`
`sudo make check -j #cores`
`sudo make install`
`sudo ldconfig`
`cd ..`
...
`cd /src/store/bftsmartstore/library`
`mkdir bin`
`ant`
...
`cd googletest`
`git checkout release-1.10.0`
`rm -rf <PATH>/src/.obj/gtest`
`mkdir <PATH>/src/.obj`
`cp -r googletest <PATH>/src/.obj/gtest`
`cd <PATH>/src/.obj/gtest`
