...
for i, hostname in enumerate(hosts_dict):
    ...
    with open("/home/floriansuri/BFT-SMART-STORE/BFT-DB/src/store/bftsmartstore/library/remote/java-config-" + hostname + "/java-config/system.config") as f:
        s = f.read()
        if "= auto" not in s:
            print("= auto does not exist in the file java-config-" + hostname)
            exit(0)
    with open("/home/floriansuri/BFT-SMART-STORE/BFT-DB/src/store/bftsmartstore/library/remote/java-config-" + hostname + "/java-config/system.config", "w") as f:
        s = s.replace("= auto", "= " + ipaddr)
        f.write(s)
        print("the content of s: ")
        print(s)
    with open("/home/floriansuri/BFT-SMART-STORE/BFT-DB/src/store/bftsmartstore/library/remote/java-config-" + hostname + "/java-config/hosts.config", "w") as f:
        k = i % 3
        for j in range(4):
