...
SyncTPCCClient::SyncTPCCClient(SyncClient &client, Transport &transport,
    uint32_t seed, int numRequests, int expDuration, uint64_t delay, int warmupSec,
...
SyncTransaction* SyncTPCCClient::GetNextTransaction() {
  ...
  switch (ttype) {
    case TXN_NEW_ORDER:
      return new SyncNewOrder(wid, C_c_id, num_warehouses, GetRand());
    case TXN_PAYMENT:
      return new SyncPayment(wid, C_c_last, C_c_id, num_warehouses, GetRand());
    case TXN_ORDER_STATUS:
      return new SyncOrderStatus(wid, C_c_last, C_c_id, GetRand());
    case TXN_STOCK_LEVEL:
      return new SyncStockLevel(wid, did, GetRand());
    case TXN_DELIVERY:
      return new SyncDelivery(wid, did, GetRand());
    default:
      NOT_REACHABLE();
  }
}
