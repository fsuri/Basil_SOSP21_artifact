...
PermBranchGenerator::PermBranchGenerator() {
...
uint64_t PermBranchGenerator::GenerateBranches(const proto::Branch &init,
...
void PermBranchGenerator::GenerateBranchesSubsets(
...
void PermBranchGenerator::GenerateBranchesPermutations(
    const std::vector<uint64_t> &txns,
    const BranchMap &p_branches,
    const SpecStore &store,
    std::vector<proto::Branch> &new_branches) {
