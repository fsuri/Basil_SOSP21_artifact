#include "store/bftsmartstore_stable/shardclient.h"
...
void ShardClient::ReceiveMessage(const TransportAddress &remote,
  ...
  if (t == signedMessage.GetTypeName()) {
    ...
    crypto::PubKey* replicaPublicKey = keyManager->GetPublicKey(
      ...
      if (!bftsmartBatchedSigs_stable::verifyBatchedSignature(signedMessage.mutable_signature(), signedMessage.mutable_packed_msg(),
            replicaPublicKey)) {
