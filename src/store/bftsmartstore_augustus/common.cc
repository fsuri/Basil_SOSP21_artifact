...
#include "store/bftsmartstore_augustus/pbft_batched_sigs.h"
          ...
          if (decision.ParseFromString(packedMsg.msg())) {
            ...
            if (decision.status() == REPLY_OK &&
              ...
              for (const auto& id_sig_pair: grouped.signatures()) {
                ...
                //   if(bftsmart_augustus_BatchedSigs::verifyBatchedSignature(sig, msg, key)){
                ...
                //   }
                // });
                if (bftsmart_augustus_BatchedSigs::verifyBatchedSignature(signedMsg.mutable_signature(),
          ...
          if (decision.ParseFromString(packedMsg.msg())) {
            ...
            if (decision.status() == REPLY_FAIL &&
              ...
              for (const auto& id_sig_pair: grouped.signatures()) {
                ...
                if (bftsmart_augustus_BatchedSigs::verifyBatchedSignature(signedMsg.mutable_signature(), signedMsg.mutable_packed_msg(), replicaPublicKey)) {
                  valid_signatures.insert(id_sig_pair.first);
                  //max_counter--;
                  //if(max_counter == 0) return true;
                } else {
            ...
            if (decision.status() == REPLY_OK && decision.txn_digest() == digest && decision.shard_id() == shard_id) {
              ...
              for (const auto& id_sig_pair: grouped.signatures()) {
                ...
                auto f = [signedMsg_ = signedMsg, verifyObj, replicaPublicKey, shard_id]() mutable {
                  ...
                  bool res = bftsmart_augustus_BatchedSigs::verifyBatchedSignature(signedMsg_.mutable_signature(),
