...
Client::Client(const transport::Configuration& config, uint64_t id, int nShards, int nGroups,
  ...
  Debug("Initializing Augustus-BFTSmart client with id [%lu] %lu", client_id, ngroups);
  ...
  Debug("Augustus-BFTSmart client [%lu] created! %lu %lu", client_id, ngroups,
      bclient.size());
}
...
void Client::Get(const std::string &key, get_callback gcb,
  ...
  transport->Timer(0, [this, key, gcb, gtcb, timeout]() {
    ...
    std::string place_holder(100, '\0'); //make a longer string
