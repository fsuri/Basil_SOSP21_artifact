...
Amalgamate::Amalgamate(const std::string &cust1, const std::string &cust2, const uint32_t timeout) : SmallbankTransaction(AMALGAMATE), cust1(cust1), cust2(cust2), timeout(timeout) {}
...
int Amalgamate::Execute(SyncClient &client) {
    ...
    if (!ReadAccountRow(client, cust1, accountRow1, timeout) || !ReadAccountRow(client, cust2, accountRow2, timeout)) {
    ...
    InsertCheckingRow(client, customerId2, balance2 + checkingRow1.checking_balance() + savingRow1.saving_balance(), timeout);
