...
\fs{Argue that blockchains are functionally a database. Way that we have been building these artifacts has been driven by the model they are embedded in rather than the db functionality. We have shoehordned the BFT solution for the database functionality. Why dont we instead let the functionality drive the foundation and make sure we can provide these functionalities in this model. Have a DB that is BFT, rather than BFT to use DB.
}
