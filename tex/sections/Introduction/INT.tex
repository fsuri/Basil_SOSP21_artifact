...
While applications in stronger failure domains \fs{(Crash Failure)} are built atop transactional Databases whose primary design concern is performance, applications in Byzantine Failure assumptions must resort to systems designed instead for fault tolerance as primary design concern and scalability as optimization concern. Instead, we argue, that the sustainable way to service applications in need to BFT is to build dedicated databases that are co-designed with byzantine resilience. \fs{people might disagree}
