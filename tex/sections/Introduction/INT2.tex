...
\three ~ they often centralize the ordering of requests at a single leader, which,  raises fairness \fs{and scalability} concerns. \la{I wonder whether we should not expand a little on these three points: they may not be clear to teh reader}. These limitations all stem from the same fallacy: the mistaken belief that building out a total order of operations is necessary for correctness. \fs{transaction limited expressiveness only partially attributtable to total order imo - ; ALSO: the 2 blockchain citations use the UTXO model, its probably a stretch to say that has to due with totally ordered based? Even with partial order one-shot models are desirable (UTXO is a oneshot in my eyes), since thats single round communication/coordination} \la{Stopped here: want to talk to Florian/Natacha}.


\fs{people build abstraction of total order on top of the physical total order. Implementation mirrors abstraction - we want to instead provide a different implementation. They KNOW that total order is limiting, its not a new idea or mistaken belief. The question is, can we do it differently. And people HAVE tried: But only for crash failure - or, only via sharding }
...
They remain, at all times, per client and per-object. Specifically, we develop a variant of multiversion timestamp ordering that \changebars{limits byzantine clients ability to strategically abort honest clients' transactions}{ precludes byzantine clients from
...
\item We introduce a novel concurrency control and recovery \fs{is this the best word} mechanism that balance the need for high-throughput in
the common case with resilience to Byzantine attacks.
\item Our system remains leaderless and exhibits linear communication complexity, unlike prior BFT systems. \fs{this describes Q/U too - but we offer transactions and liveness}
\end{itemize}
