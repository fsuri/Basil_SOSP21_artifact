...
\fs{the whole next overview is sort of redundant with the shortcoming summarised above}
...
\changebars{First, it overcomes the scalability bottleneck of a low-level
implementation based on a totally ordered BFT log by leveraging aggressive optimistic concurrency control (OCC) to support concurrent transaction
processing while maintaining the abstraction
of a serial execution.}{First, it overcomes the scalability bottleneck of a low-level
...
\changebars{Second, and unlike some protocols~\cite{Kotla07Zyzzyva,castro1999practical} that leave the ordering to the whim of a potentially Byzantine leader, \sys{} sidesteps the growing concerns~\cite{herlihy2016enhancing} about whether blockchains order transactions fairly by leaving the responsibility of driving the replication and distributed commit of a transaction to the client that proposes it. Increased client autonomy in \sys comes as the cost of higher computational requirements.}{Second, by doing away with an underlying totally ordered log, \sys{} can sidestep the growing concerns~\cite{herlihy2016enhancing} about whether blockchains order transactions fairly. Though the notion of fairness in this context has not been rigorously defined, leaving the ordering to the whim of a potentially Byzantine leader, as some protocols do ~(Kotla07Zyzzyva,castro1999practical) is intuitively problematic. In contrast, \sys{} leaves the responsibility of driving the replication and distributed commit of a transaction to the client that proposes it.}\fs{cut imo:}
 \changebars{}{and allows separate 
...
\changebars{Third, \sys{} avoids the duplicate ordering costs of transactional systems
...
protocol~\cite{zhang2015tapir,mu2016consolidating}.}{Third, \sys{} avoids the duplicate ordering costs of transactional systems
built on top of state machine replication. As Tapir noted~(zhang2015tapir,mu2016consolidating),
...
protocol.} \fs{I find the statement "limiting coordination to the validation step to be too vague and inaccurate} \fs{mention the shortcoming of using sharding for PO here} 
...
setting \changebars{where replicas may attempt to subvert read integrity, and clients may collude to sabotage commit chances of correct clients' transactions.}{First, since reading from a single local replica can no
