...
\paragraph{BFT Overheads.} \fs{should this just go into the previous graphs, like we were currently evaluating? Or remove it from the previous ones.}
...
\fs{the next 2 experiments can probably go}
...
\fs{also probably redundant to be in an explicit graph}
...
\subsection{Failures}
 \fs{still need to implement fallback}
\begin{figure}
  Scatter Plot\\
