...
- we implement Indicus in TODO lines of C++ and use CloudLab machines (what specs). 
- WAN setting with distribution as follows: (2 each in usa, asia, europe each --> f+1 reads will still read in national locality)
- Compare against a crash Failure baseline: Tapir. Use the same transport layer etc for a fair comparison. We also implement a stawman totally ordered replication scheme to evaluate the impact of partially ordering and concurrency control:

PBFt baseline variations:
- Do MVTSO or OCC with deferred structure like Indicus
- 2Pl where every read must go to the leader for ordering? (Tapir already does this, so we omit because it should be the same?)

This also simulates BFT Dur.

Lastly, we compare against Hyperledger as real-world system.
