...
%-------------------------------------------------------------------------------
Next, we discuss the concurrency control necessary to maintain byzantine serializability. The goal is to minimize abort rate. Compress execution window and validation/writeback time, so TXs happen in as atomic of a time unit as possible. --> minimizes read aborts. We use TSO to predescribe a timestamp order to reduce read/write conflicts and use multi versioning in order to reduce total conflicts. This weakens strict serializability to serializability.
