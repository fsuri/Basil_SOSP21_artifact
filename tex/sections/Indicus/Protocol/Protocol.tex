...
Given hyperparameter Read Quorum Size (RQS), a Client performs a read on given key at timestamp TS by reading from RQS different replicas. To do so, a Client sends $Read \coloneqq (key, TS)$  to $\geq RQS+f$ respective replicas, in order to guarantee $\geq RQS$ replies. \fs{this does not match the rest of the discussion- say your guaranteed to receive RQS-f}
