...
Since Indicus is leaderless, Replicas need to agree on results, rather than an Ordering. To do so, they vote. As in any democratic decision, these votes are then aggregated into a decision. In the case of Indicus, the client acts as the transaction coordinator who aggregates and relays results. In order to maintain consistency in the presence of failures, this decision must be logged prior to returning, guaranteeing that a replay of any Writeback is idempotent. This two step decision protocol resembles Two-Phase Commit. Since we cannot trust a byzantine coordinator to persist a decision, nor could we retain liveness during a crash, we delegate the decision logging to the replicas.
The voting phase requires a single round-trip to all replicas, whereas the logging phase requires at most one round-trip \fs{in Indicus5 and at most two round-trips in Indicus3.}
To describe how the protocol operates we follow a transaction through the system.:

\fbox{\begin{minipage}{21em}
\textbf{(1: C $\rightarrow$ R)}: Client sends Prepare request to the Replicas.
\end{minipage}}

Upon deciding to Commit in the Execution phase, a Client initiates Validation, by sending a message $\langle Prepare, TX, (TS) \rangle_{\sigma_c}$ to all Replicas.

\underline{Additional subtlelties}: A client may not equivocate this request. Any change to the TX state changes the TXiD and hence is treated as seperate TX. A client may choose identical timestamps for 2 TX, these ties are broken by TXID order.

\fbox{\begin{minipage}{21em}
\textbf{(2: R $\rightarrow$ C)}: Replica runs Concurrency Control Check and returns vote to Client.
\end{minipage}}
Every Replica evaluates the Transactions according to the MVTSO Concurrency Control Check (CCC), as shown in algorithm 1. This check is entirely based on local knowledge. Updates local state. Returns vote to Client.

Different replicas receive these out of order.

\underline{Additional subtlelties}: A replica updates its internal state.. Ongoing TX set, prepared set. Checks whether dependencies are resolved already, otherwise puts that thread to sleep. 
...
\fbox{\begin{minipage}{21em}
\textbf{(3a: C)}: Client returns to application. Proceeds to Writeback
\end{minipage}}

\fbox{\begin{minipage}{21em}
\textbf{(3b: C $\rightarrow$ R)}: Client sends decision to Replicas for Logging
\end{minipage}}

\fbox{\begin{minipage}{21em}
\textbf{(4: R $\rightarrow$ C)}: Replicas validate and echo decision
\end{minipage}}
