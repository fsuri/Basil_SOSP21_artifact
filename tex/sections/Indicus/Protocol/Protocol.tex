...
Given a transaction TX finalized execution state, i.e. its read-set, write-set and dependencies, and a proposed timestamp, the validation check returns \textit{Abstain/Abort} (we discuss the distinction further in section X (validation)) if a conflict has been detected. Otherwise, a replica tentatively \textit{Prepares} a transaction, making its writes visible and evaluating future transactions against it for conflicts. It then waits for necessary dependencies (uncommited writes that were read) to be resolved. If they do so successfully the check returns Commit, and otherwise un-prepares the transaction and returns Abstain/Abort. We remark, that the concurrency control check is serialized and executed atomically for each transaction.

\fs{should unprepare if dep aborts; cannot move prepare after its own dependencies are resolved, otherwise other transactions could have prepared in the meantime which would not be legal. CC check needs to be atomic, i.e. have locks for all relevant keys and be serialized.  If dep depth max 1, one could technically move the dep wait to the front; but this diminishes the chances for the transactions commit longer which is undesirable.}

%%%%%%%%%%%
We conjecture that no two transactions can have the same timestamp:
A client may however choose identical timestamps for two different transactions, these ties are broken by TXID order. (this is technically a misbehavior; can just abstain from the second one received and report TX)
%%%%%%%%%%%
...
WLOG, assume that TX1 is validated before TX2 and MVTSO-Check(TX1, TX1.TS) returns Commit. Implies that TX1 is in PreparedDB
We show by case distinction that TX2's validation cannot return Commit:
\begin{itemize}
\item \textbf{TX2} TX2 read missed TX1 write -> abort. If Tx2 ts < TX1 ts then no conflict. If Tx2 version > Tx1 ts then no conflcit


\item TX2 write not seen by Tx1 -> abort. If Tx2 ts > Tx1 then no conflict. If Tx1 version > Tx2 ts then no conflict.
\end{itemize}

We now show, that every honest clients transaction is \textit{legal}, i.e. its reads are based on committed writes.
-> reads either committed or f+1 prepared to form a dependency. Only returns commit If all deps committed.

Thus, all honest clients exerpience Serializability and thus the MTVTSO check maintains byz serializability.
\end{proof}


Everything else are extras: I.e. do not abort your own dependency (exception), dont let writes pass against read timestamps (conservative, so it is safe)


%%%%%%%%%%%%%%%%%%%%%%

1. Abstain from TX that are undesirable Timestamp wise: Either too large timestamp: need to bound the impact of byz TX. Or too small: Want to garbgage collect log. Dont accept new TX or TX for who we need the dependency proofs. (maybe we just want to vote abstain/commit for such deps without proofs.)

2. make sure all reads are legitimate: did not miss committed or prepared writes

3. make sure all writes maintain read legitimacy. Dont allow writes for currently executing reads 

Subtlelties: grant exceptions for out of order 

4. Wait on dependencies to complete. Replicas block here. Need liveness granting mechanism to not block infinitely. At least one honest replica has seen the full TX and will reply with it. Thus we did not need to return it right away, but only when needed.
Byz client cannot equivocate dependencies because the TXID is dependent on it. So DAG is consistent and fully recoverable.

5. Check outcome of dependencies and legitimacy in case they retried. (probably omit from base version).
...
Upon receiving commits/aborts it checks whether blocked dependencies can wake up.
...
Goal of validation: maintain isolation even though replicas are out of order. In total order protocols agreement is expected, for us it is not, yet we need to maintain correctness.
...
Below, we detail the protocol by following a transactions life cycle. To start the protocol we assume an \textit{interested} client is in possession of the respective transactions $Prepare$ message, signed by the orignial issuer. \fs{this info can be obtained as part of MVTSO check for own Tx or through an abort; it limits how clients become interested. client learns about full PrepareTX in MVTSO check: returned to it for all TX that have not completed yet. This avoids unecessary info in the exec phase (i.e. reads including full TX)}. For sake of exposition we moreover assume that the transaction has not finalized via Writeback on any replica. We briefly discuss this case afterwards.
...
\fbox{\begin{minipage}{21em}
\textbf{(1: R $\rightarrow$ C)}:  Replicas receive and acknowledge Writeback decision
\end{minipage}}
...
\fbox{\begin{minipage}{21em}
\textbf{(2: C $\rightarrow$ R)}:  Client aggregates acknowledgements and forwards them
\end{minipage}}
The client waits for $2f+1$ matching decisions, bundles them, and forwards them to all replicas.

\fbox{\begin{minipage}{21em}
\textbf{(3: R ($\rightarrow$ R))}:  Replica receives garbage collection confirmation
\end{minipage}}
...
\textbf{Receiving Shard decisions}
