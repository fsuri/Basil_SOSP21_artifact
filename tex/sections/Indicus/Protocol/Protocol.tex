...
1) A client receives the necessary information to proceed to Writeback. We consider two subcases:\\
...
2) A client needs to go Slow-Path or observes inconsistency and requires assistance for resolution:\\
...
\fbox{\begin{minipage}{21em}
\textbf{(4a: R $\rightarrow$ C)}:  Replicas receive $Phase2$ messages and reply
\end{minipage}}
...
\fbox{\begin{minipage}{21em}
\textbf{(4b: R $\rightarrow$ R)}:  Replica additionally receives Fallback invocation and starts election
\end{minipage}}

If a replica receives an $electFB$ message and the original client has timed-out it attempts to elect a new \textit{Fallback replica}. To do so, they follow the \textbf{View Change Rules} and adopts new view $current.view = v$ if $v > current.view$. Replicas send an $Elect \coloneqq (TxID, decision, current.view)_R$ to replica $current.view + TxID \% n$.
...
\end{minipage}}
need decisions from matching view.
...
THeorem: logged decision is preserved
proof: by induction, does not need view of decision.

effectively when a replica adopts a view, it ignores all older views. Trivial, since it only accepts the first p2 and then never changes. Only thing that changes this is a FB message from higher view.
...
We remark also, that the orignial client becomes an interested client itself automatically. (If it was slow, others might get the chance to interject, but this client will still be interested in finishing the TX).

So in total if it goes badly: 1 rr voting, 3 rr FB (+1 wb) for dependencies (can be in parallel for all), 1 rr logging.



Next we show how to create a live round robin that is client driven: Liveness of mechanism: view rule, replica all to all.
We do not want replicas to just keep view changing as in normal SMR protocols. This does not work because there is no notion of progress for replicas. (we add the practical optimization of certificates so they can stop). View changes would happen when nobody cared about them, wasted work/network overhead.
...
- keep low watermark. Abstain from all TX blow this watermark or TX with dependencies below this watermark. (Byz replicas can still vote abort with the proof if they have it, speeds things up).
Remove proofs for commit/abort 
%-------------------------------------------------------------------------------
